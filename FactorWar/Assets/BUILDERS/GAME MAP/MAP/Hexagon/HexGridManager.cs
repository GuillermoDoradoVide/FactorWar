...
public class HexGridManager : MonoBehaviour {
    ...
    public static HexGridManager Instance
    {
        get
        {
            if (Quitting)
            {
                Debug.LogWarning(typeof(HexGridManager).ToString() + ", Instance will not be returned, Application is Quitting.");
                return null;
            }
            ...
            lock (Lock)
            {
                ...
                var objs = FindObjectsOfType<HexGridManager>();
                if (objs.Length > 0)
                {
                    if (objs.Length == 1)
                        return _instance = objs[0];

                    Debug.LogWarning(typeof(HexGridManager).ToString() + ", More than 1 Singleton! First intance used, all others destroyed.");

                    for (var i = 1; i < objs.Length; i++)
                        Destroy(objs[i]);
                    return _instance = objs[0];
                }
                GameObject obj = new GameObject(typeof(HexGridManager).ToString());
                DontDestroyOnLoad(obj);
                return _instance = obj.AddComponent<HexGridManager>();
            }
        }
    }
